...
class StepSequencerDisplay(displayio.Group):
	def __init__(self, sequencer):
		...
		for (x,y) in step_text_pos:
			self.stepgroup.append( label.Label(terminalio.FONT, text="txt ", x=x, y=y, line_spacing=0.65))
		...
		self.tempo_text = label.Label(terminalio.FONT, text="tmpo", x=tempo_text_pos[0], y=tempo_text_pos[1])
		self.play_text = label.Label(terminalio.FONT, text="play", x=play_text_pos[0], y=play_text_pos[1])
		self.transpose_text = label.Label(terminalio.FONT, text="trans", x=trans_text_pos[0], y=trans_text_pos[1])
	...
	def update_ui_step(self, step, n, v=127, gate=8, on=True, selected=False):
		...
		notestr = self.seq.notenum_to_name(n)	# FIXME: requires seq.notenum_to_name hack
